...
class GcloudASR:
    def __init__(self, options):
        ...
        if self._is_multilingual:
            _config.update({
                "alternative_language_codes": self._opt_langs
            })
        if self._is_word_conf:
            _config.update({
                "enable_word_confidence": self._is_word_conf
            })
        if self._is_punct:
            _config.update({
                "enable_automatic_punctuation": self._is_punct
            })
...
class ASRBot:
    def __init__(self, tg_api_key: str, options: dict, messages: dict, recognizer: GcloudASR):
